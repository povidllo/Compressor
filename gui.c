...
#define PATH "UI_files/main_UI.glade"
...
//gcc -O2 -Wl,-rpath=. -o gui gui.c `pkg-config --cflags --libs gtk+-3.0` -rdynamic
...
G_MODULE_EXPORT void zip_button_clicked_cb(GtkButton * btn, gpointer data)  //open zip_menu window
{
    GtkBuilder * ui_builder;
    GError * err = NULL;
    ui_builder = gtk_builder_new();
    if(!gtk_builder_add_from_file(ui_builder, PATH, &err)) {
...
G_MODULE_EXPORT void unzip_button_clicked_cb(GtkButton * btn, gpointer data)  //open unzip_menu window
{
    GtkBuilder * ui_builder;
    GError * err = NULL;
    ui_builder = gtk_builder_new();
    ...
    GtkWidget * window = GTK_WIDGET(gtk_builder_get_object(ui_builder, "unzip_menu"));
    gtk_builder_connect_signals(ui_builder, NULL);
    gtk_widget_show_all(window);
}
